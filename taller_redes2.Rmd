...
La centralidad de un nodo es una métrica que permite identificar la _importancia_ de un nodo en la red. Claramente, la importancia depende del análisis que el usuario haga de la red, por lo que hay varias medidas de centralidad. Una medida típica de centralidad es el grado (interno, externo o total) de un nodo, bajo el supuesto de que un nodo con más conexiones es más relevante en la red.

Otras métricas incluyen la __cercanía__, la cual está basada en la distancia de un nodo a todos los demás de la red. Se calcula como el inverso de la distancia geodésica promedio entre el nodo y todos los demás.

```{r}
closeness(net, mode="all", weights=NA) 
```

El __betweenness__ es una medida de centralidad que intenta caracterizar el efecto que tendría en la red la remoción del nodo o el arco. El betweenness se define como la proporción de distancias geodésicas que pasan por cada nodo o arco.
...
En R hay varias funciones que nos permiten calcular distancias y caminos entre nodos. Las funciones ```mean_distance``` y ```distance``` calculan la distancia geodésica promedio y la distancia entre cada par de nodos respectivamente. En cada función se puede especificar el parámetro ```directed``` para indicar si la red es dirigida o no.
...
Es posible también especificar un subconjunto de nodos en los que estemos interesados y una direccionalidad. En el siguiente ejemplo calcularemos la distancia de cada uno de los medios al New York Times:

```{r fig.width=8}
dist.from.NYT <- distances(net, v=V(net)[media=="NY Times"], to=V(net), weights=NA)

# Asignamos los colores según la distancia:
...
También se puede calcular el camino más corto entre un par específico de nodos. En el ejemplo calculamos el camino más corto entre el New York Times y CNN:
...
# Generar los colores para los arcos, los del camino serán naranjos:
...
# Los nodos del camino serán dorados:
...
Finalmente, se pueden encontrar las aristas y nodos adyacentes a algún nodo:
...
```{r fig.width=8}
# Se encuentran los nodos adyacentes, con la dirección indicada en mode
...
Luego graficamos la comunidad en base al grafo obtenido con ciertas opciones como agregar el nombre de cada medio noticioso, tamaño de las flechas, etc.
```{r fig.width= 9}
plot(ceb, net.wl, edge.arrow.size = 0.2, vertex.label = V(net.wl)$media, vertex.label.color = "black", vertex.label.dist = 2) 

```

Un segundo algoritmo que también podemos utilizar para encontrar comunidades es el basado en modularidad. Este método busca conexiones densas dentro de cada comunidad tratando de maximizar a su vez la separación distintos sub grafos.
```{r fig.width= 10}
cfg <- cluster_fast_greedy(net.sym)

plot(cfg, net.sym, edge.arrow.size = 0.2, vertex.label = V(net.wl)$media, vertex.label.color = "black", vertex.label.dist = 2)
```

# Taller Práctico

Utilizaremos un dataset que contiene los proyectos de ley presentados por diputados durante el año 2006. Cada nodo representa una persona y los arcos el boletín (proyecto) presentado por ambos. Para ello, cargaremos los datasets ``netsponsor_2006_1mes.csv`` y ``vertex_2006_1mes.csv``.
...
Así como también sus atributos
```{r}
edge_attr(netsponsor)
vertex_attr(netsponsor)
```

Primero, visualizaremos el grafo sin mayor procesamiento ya que solo quitaremos los labels de los nodos y ajustaremos el tamaño del vértice y arco.
```{r fig.width= 10}
...
También podemos obtener las comunidades que se generan a partir del principal utilizando el algoritmo de **Newman-Girvan**.
```{r}
...
Para terminar, calcularemos algunas medidas analíticas de la red, como su distancia promedio y su diámetro:
...
ecol[E(netsponsor, path=diam)] <- "tomato" # Pintamos rojos los arcos del diámetro
...
Finalmente, obtendremos los nombres de quienes son más importantes para la red, calculando las medidas de centralidad
...
```{r}
# La cercanía no está definida en redes desconectadas, entonces obtenemos la componente más grande
